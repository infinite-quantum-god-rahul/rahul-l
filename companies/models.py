...
class UserCreation(BaseRaw):
    """
    Clean User Creation model - avoids naming conflicts with Django internals
    """
    user = models.ForeignKey(
        AuthUser, null=True, blank=True, on_delete=models.SET_NULL,
        related_name="user_creation_profile"
    )
    staff = models.ForeignKey(
        Staff, on_delete=models.CASCADE, related_name="user_creations", null=True, blank=True
    )
    full_name = models.CharField(max_length=255, default="Unnamed User")
    branch = models.ForeignKey(
        Branch, on_delete=models.CASCADE, related_name="user_creations", null=True, blank=True
    )
    ...
    class Meta:
        db_table = 'companies_user_creation'
        verbose_name = 'User Creation'
        verbose_name_plural = 'User Creations'
    
    def __str__(self):
        return f"{self.full_name} ({self.staff.name if self.staff else 'No Staff'})"
