#!/usr/bin/env python
"""
HTTPS Django Development Server
==============================

This script runs Django with HTTPS support using a self-signed certificate.
"""

import os
import sys
import django
from django.core.management import execute_from_command_line
from django.core.servers.basehttp import WSGIServer
from django.core.wsgi import get_wsgi_application
import ssl
import socket

def run_https_server():
    """Run Django with HTTPS support"""
    
    # Set up Django
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'spoorthi_macs.settings')
    django.setup()
    
    # Get WSGI application
    application = get_wsgi_application()
    
    # Create a simple HTTP server that we'll wrap with SSL
    from http.server import HTTPServer
    from django.core.handlers.wsgi import WSGIHandler
    
    class WSGIHTTPServer(HTTPServer):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.wsgi_app = WSGIHandler()
    
    # Create server
    server_address = ('127.0.0.1', 8001)
    httpd = WSGIHTTPServer(server_address, lambda *args, **kwargs: None)
    
    # Create SSL context
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
    
    # Generate a self-signed certificate
    import tempfile
    import subprocess
    
    # Create temporary certificate files
    with tempfile.NamedTemporaryFile(mode='w', suffix='.key', delete=False) as key_file:
        key_path = key_file.name
    
    with tempfile.NamedTemporaryFile(mode='w', suffix='.crt', delete=False) as cert_file:
        cert_path = cert_file.name
    
    try:
        # Generate self-signed certificate using OpenSSL
        subprocess.run([
            'openssl', 'req', '-x509', '-newkey', 'rsa:4096', '-keyout', key_path,
            '-out', cert_path, '-days', '365', '-nodes', '-subj',
            '/C=US/ST=State/L=City/O=Organization/CN=127.0.0.1'
        ], check=True, capture_output=True)
        
        # Load certificate
        context.load_cert_chain(cert_path, key_path)
        
        # Wrap socket with SSL
        httpd.socket = context.wrap_socket(httpd.socket, server_side=True)
        
        print("HTTPS Django server running at https://127.0.0.1:8001/")
        print("Note: You may see a security warning - click 'Advanced' and 'Proceed'")
        print("Press Ctrl+C to stop the server")
        
        # Start server
        httpd.serve_forever()
        
    except subprocess.CalledProcessError:
        print("OpenSSL not found. Trying alternative method...")
        
        # Alternative: Use a simple certificate generation
        try:
            from cryptography import x509
            from cryptography.x509.oid import NameOID
            from cryptography.hazmat.primitives import hashes, serialization
            from cryptography.hazmat.primitives.asymmetric import rsa
            import datetime
            
            # Generate private key
            private_key = rsa.generate_private_key(
                public_exponent=65537,
                key_size=2048,
            )
            
            # Create certificate
            subject = issuer = x509.Name([
                x509.NameAttribute(NameOID.COUNTRY_NAME, "US"),
                x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, "State"),
                x509.NameAttribute(NameOID.LOCALITY_NAME, "City"),
                x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Organization"),
                x509.NameAttribute(NameOID.COMMON_NAME, "127.0.0.1"),
            ])
            
            cert = x509.CertificateBuilder().subject_name(
                subject
            ).issuer_name(
                issuer
            ).public_key(
                private_key.public_key()
            ).serial_number(
                x509.random_serial_number()
            ).not_valid_before(
                datetime.datetime.utcnow()
            ).not_valid_after(
                datetime.datetime.utcnow() + datetime.timedelta(days=365)
            ).add_extension(
                x509.SubjectAlternativeName([
                    x509.DNSName("127.0.0.1"),
                    x509.DNSName("localhost"),
                ]),
                critical=False,
            ).sign(private_key, hashes.SHA256())
            
            # Save certificate and key
            with open(cert_path, "wb") as f:
                f.write(cert.public_bytes(serialization.Encoding.PEM))
            
            with open(key_path, "wb") as f:
                f.write(private_key.private_bytes(
                    encoding=serialization.Encoding.PEM,
                    format=serialization.PrivateFormat.PKCS8,
                    encryption_algorithm=serialization.NoEncryption()
                ))
            
            # Load certificate
            context.load_cert_chain(cert_path, key_path)
            
            # Wrap socket with SSL
            httpd.socket = context.wrap_socket(httpd.socket, server_side=True)
            
            print("HTTPS Django server running at https://127.0.0.1:8001/")
            print("Note: You may see a security warning - click 'Advanced' and 'Proceed'")
            print("Press Ctrl+C to stop the server")
            
            # Start server
            httpd.serve_forever()
            
        except ImportError:
            print("Cryptography library not available. Using simple method...")
            
            # Simple method: Just run with HTTP but on a different port
            print("Running Django on HTTP port 8002 instead...")
            os.system("python manage.py runserver 127.0.0.1:8002")
    
    finally:
        # Clean up temporary files
        try:
            os.unlink(key_path)
            os.unlink(cert_path)
        except:
            pass

if __name__ == '__main__':
    run_https_server()

